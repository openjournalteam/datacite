...
<div class="listPanel__footer">
    <div style="display: flex;align-items: center;">
        ...
        <button 
            ...
            {if $canClickPrevious} 
                hx-get="{url page="management" op="importexport" path=['plugin', $plugin, 'depositedtab'] params=['page' => $previousPage, 'querySearch' => $querySearch]}" 
                hx-target="#deposited-tab-content"
                hx-disabled-elt="this" 
                {* hx-replace-url="{url page="management" op="importexport" path=['plugin', $plugin] params=['depositedtab' => $previousPage]}" *}
            {/if}
            >
            Previous
        </button>
        </div>
        <div style="font-size:14px;color:#747474;flex: 1 1 0%;display:grid;justify-content: center;">
            <span style="text-align: center;">{$offset} - {$itemsShowed} of {$submissionCounts} Submissions</span>
        </div>
        <div style="display: flex;align-items: center; gap: 4px;">
            <button 
                ...
                {if $canClickNext} 
                    hx-get="{url page="management" op="importexport" path=['plugin', $plugin, 'depositedtab'] params=['page' => $nextPage, 'querySearch' => $querySearch]}"
                    hx-target="#deposited-tab-content"
                    hx-disabled-elt="this" 
